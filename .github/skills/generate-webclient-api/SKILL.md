...
description: 'Scaffold or extend WebClient feature API services from endpoint files (e.g., Warehouse.cs). Use when creating src/features/[feature]/api/[feature].service.ts and index.ts with @/lib/axios, no try/catch, and endpoint-aligned methods.'
...
3. Generate or update `src/features/[feature]/api/[feature].service.ts`:
   ...
   - Add documentation comments summarizing the endpoint purpose and inputs (see in canonical doc for format).
   - Do not return res.data. Let callers handle the response (refer to canonical doc).
4. Generate or update `src/features/[feature]/api/index.ts`:
