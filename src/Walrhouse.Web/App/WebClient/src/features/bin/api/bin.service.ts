 ...
 * POST /api/Bin
 */
export const createBin = (data: CreateBinRequest) => api.post<string>('/api/Bin', data);

/**
 * Get paginated list of bins.
 * GET /api/Bin
 */
export const getBins = (params?: GetBinsRequest) => api.get('/api/Bin', { params });

/**
 * Get a single bin by bin number.
 * GET /api/Bin/{binNo}
 */
export const getBin = (binNo: string) => api.get(`/api/Bin/${binNo}`);

/**
 * Update an existing bin.
 * PUT /api/Bin/{binNo}
 */
export const updateBin = (binNo: string, data: UpdateBinRequest) =>
  api.put(`/api/Bin/${binNo}`, data);

/**
 * Soft-delete a bin by bin number.
 * DELETE /api/Bin/{binNo}
 */
export const deleteBin = (binNo: string) => api.delete(`/api/Bin/${binNo}`);
